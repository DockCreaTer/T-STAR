...
class Server{
	 ...
	 * @return string
	 */
	public function getTsrApiVersion(){
		return \pocketmine\TSTAR_API_VERSION;
	}

	/**
	 * @return string
	 */
	public function getTstarApiVersion(){
		return \pocketmine\TSTAR_API_VERSION;
	}
	...
	伺服端版本: §6' . $this->getPocketMineVersion() . '§f
	用戶端版本: §b' . \pocketmine\MINECRAFT_VERSION . '§f
	源碼連結: §dhttps://github.com/DockCreaTer/T-STAR§f
	';
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath, $defaultLang = "unknown"){
		...
		try{
			...
			if(!file_exists($this->dataPath . "T-STAR.yml")){
				@file_put_contents($this->dataPath . "T-STAR.yml", $content);
			}
			$internelConfig = new Config($file, Config::YAML, []);
			$this->advancedConfig = new Config($this->dataPath . "T-STAR.yml", Config::YAML, []);
			...
			if(!extension_loaded("openssl")){
				$this->logger->warning("The OpenSSL extension is not loaded, and this is required for XBOX authentication to work. If you want to use Xbox Live auth, please update your PHP binaries at tsr.tw/download, or recompile PHP with the OpenSSL extension.");
			...
			if($this->netherEnabled){
				if(!$this->loadLevel($this->netherName)){
					//$this->logger->info("正在生成地域 ".$this->netherName);
			...
			if($cfgVer > $advVer){
				$this->logger->notice("您的 T-STAR.yml 需要升級");
				$this->logger->notice("現在所用版本: $advVer   最新版本: $cfgVer");
			}
